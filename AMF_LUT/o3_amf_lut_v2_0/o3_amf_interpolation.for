...
C  Version 2.0 created on 30/10/2009 by BIRA-IASB
C  Contact: François Hendrick (franch@oma.be)
	...
	INTEGER n_sza_int,n_o3,n_alb,n_alt,ind_lat,cc,inter_flag,dd
	INTEGER screen_flag
      PARAMETER (n_wv=5,n_lat=18,n_month=12,n_sza=13,n_sza_max=500000)
	PARAMETER (n_o3=10,n_alb=2,n_alt=2)
	PARAMETER (n_ind_low=10,n_ind_up=10)
	INTEGER ind_low(n_ind_low),ind_up(n_ind_low),albedo_flag
	INTEGER month_nb
	REAL*8 wv_vec(n_wv),lat_vec(n_lat),sza_vec(n_sza),alb_vec(n_alb)
	REAL*8 alt_vec(n_alt),o3_scd_effective(n_wv,n_o3)
	REAL*8 o3_amf(n_wv),o3_amf_wv
	...
	REAL*8 wvl_int,lat_int,sza_int(n_sza_max),ycalc,hh
	REAL*8 scd_int(n_sza_max),alb_int,alt_int,o3_col_int
	REAL*8 vcd_int(n_sza_max),dycalc,long_int,daynb_int(n_sza_max)
	REAL*8 o3_vcd(n_o3),daynb_int_out,lat_int_out,o3_vcd_vec(n_o3)
	REAL*8 o3_amftbl_lat(n_alt,n_alb,n_wv,n_month,n_sza,n_o3)
	REAL*8 o3_scdtbl_lat(n_alt,n_alb,n_wv,n_month,n_sza,n_o3)
	REAL*8 o3_amftbl_dn(n_alb,n_wv,n_sza,n_o3)
	REAL*8 o3_amftbl_alt(n_alb,n_wv,n_month,n_sza,n_o3)
	REAL*8 o3_amftbl_alb(n_wv,n_sza,n_o3)
	...
	REAL*8 alb_int_11,alb_int_12
	REAL*8 o3_scdtbl_dn(n_alb,n_wv,n_sza,n_o3)
	REAL*8 o3_scdtbl_alt(n_alb,n_wv,n_month,n_sza,n_o3)
	REAL*8 o3_scdtbl_alb(n_wv,n_sza,n_o3),alb_mat_lat(360)
	REAL*8 o3_scdtbl_wv(n_lat,n_sza,n_o3),alb_mat(360,180)
	REAL*8 o3_amf_effective(n_wv,n_o3)
	REAL*8 o3_amftbl_sza_int(n_wv,n_o3),long_alb_vec(360)
	REAL*8 o3_scdtbl_sza_int(n_wv,n_o3),lat_alb_vec(180)
	REAL*8 YWORK(50),Y2(50),lat_vec_work(2)
...
C read SZA file
	...
	IF(inter_flag.EQ.2) THEN
        READ(1,*,END=19) daynb_int(i),sza_int(i),scd_int(i)
	ELSE
        READ(1,*,END=19) daynb_int(i),sza_int(i),vcd_int(i)
	ENDIF
...
c Interpoaltion on the latitude
	...
	DO i_alt=1,n_alt
		DO i_alb=1,n_alb
			DO i_wv=1,n_wv
				DO i_month=1,n_month
					DO i_sza=1,n_sza
						DO i_o3=1,n_o3
	    IF (o3_scdtbl(i_alt,i_alb,i_wv,1,i_month,1,i_o3).LT.1.d17) 
     $      THEN	    
						ind_low(i_o3)=1
					ELSE
						ind_low(i_o3)=0
          ...
          IF (o3_scdtbl(i_alt,i_alb,i_wv,2,i_month,1,i_o3).LT.1.d17) 
     $      THEN
			ind_up(i_o3)=1
		ELSE 
              ind_up(i_o3)=0
     ...
     $	o3_amftbl(i_alt,i_alb,i_wv,:,i_month,i_sza,i_o3),ycalc,hh)
		o3_amftbl_lat(i_alt,i_alb,i_wv,i_month,i_sza,i_o3)=ycalc
			CALL inter(2,2,2,lat_int,lat_vec_work,
     $	o3_scdtbl(i_alt,i_alb,i_wv,:,i_month,i_sza,i_o3),ycalc,hh)
		o3_scdtbl_lat(i_alt,i_alb,i_wv,i_month,i_sza,i_o3)=ycalc

          IF(lat_int.GE.0) THEN
			IF(ABS(lat_int-lat_vec(2)).LE.5) THEN
	  IF(ind_low(i_o3).EQ.1.AND.ind_up(i_o3).EQ.1) THEN
			o3_amftbl_lat(i_alt,i_alb,i_wv,i_month,i_sza,i_o3)=999.d0
	        o3_scdtbl_lat(i_alt,i_alb,i_wv,i_month,i_sza,i_o3)=999.d0
	  ELSEIF (ind_low(i_o3).EQ.1.AND.ind_up(i_o3).NE.1) THEN
	   o3_amftbl_lat(i_alt,i_alb,i_wv,i_month,i_sza,i_o3)=
     $ o3_amftbl(i_alt,i_alb,i_wv,2,i_month,1,i_o3)
	   o3_scdtbl_lat(i_alt,i_alb,i_wv,i_month,i_sza,i_o3)= 
     $ o3_scdtbl(i_alt,i_alb,i_wv,2,i_month,1,i_o3)
	  ENDIF
			ELSE
	  ...
	  ELSEIF (ind_low(i_o3).NE.1.AND.ind_up(i_o3).EQ.1) THEN
	   o3_amftbl_lat(i_alt,i_alb,i_wv,i_month,i_sza,i_o3)=
     $ o3_amftbl(i_alt,i_alb,i_wv,1,i_month,1,i_o3)
	   o3_scdtbl_lat(i_alt,i_alb,i_wv,i_month,i_sza,i_o3)= 
     $ o3_scdtbl(i_alt,i_alb,i_wv,1,i_month,1,i_o3)
				ENDIF
			ENDIF
...
c     Interpolation on the altitude
     ...
     $	o3_amftbl_lat(:,i_alb,i_wv,i_month,i_sza,i_o3),ycalc,hh)
		o3_amftbl_alt(i_alb,i_wv,i_month,i_sza,i_o3)=ycalc
	            CALL inter(n_alt,n_alt,2,alt_int,alt_vec,
     $	o3_scdtbl_lat(:,i_alb,i_wv,i_month,i_sza,i_o3),ycalc,hh)
		o3_scdtbl_alt(i_alb,i_wv,i_month,i_sza,i_o3)=ycalc
					ENDDO
				ENDDO
			ENDDO
		ENDDO
...
C  Write parameters values in the output file
	...
	IF(inter_flag.EQ.2) THEN
	    WRITE(1,205) '% Day number    SZA (°)  O3 SCD (molec/cm2)
     $ O3 AMF'
	ELSE
          WRITE(1,208) '% Day number   SZA (°)  O3 VCD (DU)  O3 AMF'
	ENDIF
...
C     Interpolation on the day number
		DO i_alb=1,n_alb
			DO i_wv=1,n_wv
					DO i_sza=1,n_sza
						DO i_o3=1,n_o3
          CALL SPLINE(daynb_vec,o3_amftbl_alt(i_alb,i_wv,:,
     $	     i_sza,i_o3),n_month,YWORK,Y2)
	    CALL SPLINT(daynb_vec,o3_amftbl_alt(i_alb,i_wv,:,
     $		i_sza,i_o3),Y2,n_month,daynb_int(dd),ycalc)
		o3_amftbl_dn(i_alb,i_wv,i_sza,i_o3)=ycalc
          CALL SPLINE(daynb_vec,o3_scdtbl_alt(i_alb,i_wv,:,
     $	     i_sza,i_o3),n_month,YWORK,Y2)
	    CALL SPLINT(daynb_vec,o3_scdtbl_alt(i_alb,i_wv,:,
     $		i_sza,i_o3),Y2,n_month,daynb_int(dd),ycalc)
		o3_scdtbl_dn(i_alb,i_wv,i_sza,i_o3)=ycalc
						ENDDO
					ENDDO
			ENDDO
		ENDDO
...
C     Interpolation on the albedo
	DO i_wv=1,n_wv
     ...
     $	o3_amftbl_dn(:,i_wv,i_sza,i_o3),ycalc,hh)
		o3_amftbl_alb(i_wv,i_sza,i_o3)=ycalc
					CALL inter(n_alb,n_alb,2,alb_int,alb_vec,
     $	o3_scdtbl_dn(:,i_wv,i_sza,i_o3),ycalc,hh)
		o3_scdtbl_alb(i_wv,i_sza,i_o3)=ycalc
				ENDDO
			ENDDO
	ENDDO
...
c     Interpolation on the SZA
      DO i_wv=1,n_wv
	DO i_o3=1,n_o3
	    CALL SPLINE(sza_vec,o3_amftbl_alb(i_wv,:,i_o3),n_sza,YWORK,Y2)
		CALL SPLINT(sza_vec,o3_amftbl_alb(i_wv,:,i_o3),Y2,n_sza,
     $      			sza_int(dd),ycalc)
	    o3_amftbl_sza_int(i_wv,i_o3)=ycalc
          CALL SPLINE(sza_vec,o3_scdtbl_alb(i_wv,:,i_o3),n_sza,YWORK,Y2)
	    CALL SPLINT(sza_vec,o3_scdtbl_alb(i_wv,:,i_o3),Y2,n_sza,
     $      			sza_int(dd),ycalc)
		o3_scdtbl_sza_int(i_wv,i_o3)=ycalc
	ENDDO
	ENDDO
...
c Interpolation on O3 SCD in molec/cm2
			...
			cc=1
			DO i_o3=1,n_o3
				IF(o3_amftbl_sza_int(i_wv,i_o3).LT.997.d0) THEN
			 o3_scd_effective(i_wv,cc)=o3_scdtbl_sza_int(i_wv,i_o3)
	         o3_amf_effective(i_wv,cc)=o3_amftbl_sza_int(i_wv,i_o3)
	            cc=cc+1
				ENDIF
			ENDDO

              CALL inter(cc-1,cc-1,2,scd_int(dd),
     $	    o3_scd_effective(i_wv,1:cc-1),
     $			o3_amf_effective(i_wv,1:cc-1),ycalc,hh)
	        o3_amf(i_wv)=ycalc
	        ENDDO
	ELSE
c	Interpolation on O3 VCD in DU
	       ...
	       CALL inter(cc-1,cc-1,2,vcd_int(dd),
     $  o3_vcd_effective(1:cc-1),o3_amf_effective(i_wv,1:cc-1),ycalc,hh)
             o3_amf(i_wv)=ycalc
	       ENDDO
	ENDIF
...
C     Interpolation on the wavelength
          CALL SPLINE(wv_vec,o3_amf(:),n_wv,YWORK,Y2)
	    CALL SPLINT(wv_vec,o3_amf(:),Y2,n_wv,wvl_int,ycalc)
		o3_amf_int=ycalc
	...
	IF(inter_flag.EQ.2) THEN
	    WRITE(1,206) daynb_int_out, sza_int(dd),scd_int(dd),
     $         o3_amf_int
	ELSE
          WRITE(1,207) daynb_int_out, sza_int(dd),vcd_int(dd),
     $         o3_amf_int
	ENDIF

      IF(screen_flag.EQ.1) THEN
	WRITE(*,*) daynb_int_out, sza_int(dd), o3_amf_int
	WRITE(*,*) ''
	ENDIF

	ENDDO !day number loop
...
204   FORMAT(a36,F4.2)
205   FORMAT(a50)
206   FORMAT(F10.2,x,F10.2,8x,E10.4,x,F10.3)
207   FORMAT(F10.2,x,F10.2,x,F10.1,x,F10.3)
208   FORMAT(a43)
