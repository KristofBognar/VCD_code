function [dscd_S, qdoas_filt, qdoas_raw] = read_QDOAS_v2016(file_nm, col, filt,version,save_fig,working_dir)
...
if nargin == 4 % the default setting won't save the figure
    ...
    plot(qdoas_filt(:,col.fd), qdoas_filt(:, col.dscd), '.')
    legend('Raw', 'Filt')
    xlabel('day')
    ...
    plot(qdoas_filt(:,col.fd), qdoas_filt(:, col.rms), '.')
    legend('Raw', 'Filt')
    xlabel('day')
    ylabel('RMS')
    subplot(3,1,3)
    ...
    plot(qdoas_filt(:,col.fd), qdoas_filt(:, col.sza), '.')
    legend('Raw', 'Filt')
    xlabel('day')
    ylabel('SZA')
    cd(working_dir);
